...
from telethon.tl.functions.auth import (
    ResetAuthorizationsRequest, CheckPasswordRequest,
    ExportLoginTokenRequest, ImportLoginTokenRequest,
)
...
_pending_session_rotations = {}  # stock_id -> {"s2_client": TelegramClient, "phone": str, "started": float}
...
def init_db():
      ...
      with db_conn() as c:
          ...
          for _alt in [
              ...
              "ALTER TABLE number_stock ADD COLUMN IF NOT EXISTS session_rotation_pending BOOLEAN DEFAULT FALSE",
...
async def _solve_captcha_with_gemini(text: str = None, image_bytes: bytes = None) -> str | None:
    """يستخدم Gemini لاستخراج رقم التحقق من نص أو صورة. يُرجع الرقم فقط أو None عند الفشل."""
    ...
    try:
        if image_bytes:
            ...
            payload = {"contents": [{"parts": [
                {"text": "استخرج رقم التحقق من هذه الصورة. أرجع الأرقام فقط بدون أي كلام آخر."},
                {"inline_data": {"mime_type": "image/jpeg", "data": b64_data}}
            ]}]}
        elif text:
            payload = {"contents": [{"parts": [
                {"text": (
                    "استخرج رقم التحقق من النص التالي. "
                    "أرجع الأرقام فقط بدون أي كلام آخر.\n\n" + text
                )}
            ]}]}
        else:
            return None
        ...
        logger.warning(f"⚠️ Gemini captcha error: {_ge}")
        return None
...
async def retry_pending_session_rotations_job(context: ContextTypes.DEFAULT_TYPE):
    """
    مهمة دورية تُكمل المرحلة 4 من تدوير الجلسة (ResetAuthorizations) للأرقام التي
    حُفظت جلستها الجديدة S2 في DB لكن لم يتم طرد S1 بعد (session_rotation_pending=TRUE).
    """
    if not (TELEGRAM_API_ID and TELEGRAM_API_HASH):
        return
    with db_conn() as c:
        rows = c.execute(
            "SELECT id, phone_number, session_string FROM number_stock "
            "WHERE session_rotation_pending = TRUE AND session_string IS NOT NULL AND deleted_at IS NULL"
        ).fetchall()
    for row in rows:
        rec = dict(row)
        s2 = TelegramClient(StringSession(rec["session_string"]), int(TELEGRAM_API_ID), TELEGRAM_API_HASH)
        try:
            await asyncio.wait_for(s2.connect(), timeout=15)
            if not await asyncio.wait_for(s2.is_user_authorized(), timeout=8):
                # الجلسة الجديدة S2 أصبحت غير صالحة — نلغي العلامة
                with db_conn() as c2:
                    c2.execute(
                        "UPDATE number_stock SET session_rotation_pending=FALSE WHERE id=%s",
                        (rec["id"],)
                    )
                logger.warning(f"⚠️ rotation retry: S2 غير مصرّح للرقم {rec['phone_number']} — تم إلغاء العلامة")
                continue
            # المرحلة 4: S2 تقتل S1 وكل الجلسات الأخرى
            await s2(ResetAuthorizationsRequest())
            _dev_cnt = -1
            try:
                _dev_cnt = await get_device_count(s2)
            except Exception:
                pass
            _is_solo = (_dev_cnt == 1)
            with db_conn() as c2:
                c2.execute(
                    "UPDATE number_stock SET session_rotation_pending=FALSE, sessions_reset=TRUE, is_solo=%s WHERE id=%s",
                    (_is_solo, rec["id"])
                )
            if OWNER_ID:
                await context.bot.send_message(
                    OWNER_ID,
                    f"🔄✅ *اكتمل تدوير الجلسة (إعادة محاولة تلقائية)*\n\n"
                    f"📱 الرقم: `{rec['phone_number']}`\n"
                    f"🔒 الجلسة القديمة S1 أُتلفت — S2 الجديدة نشطة وحدها.",
                    parse_mode=ParseMode.MARKDOWN
                )
            logger.info(f"🔄✅ rotation retry: اكتمل تدوير الجلسة للرقم {rec['phone_number']} | is_solo={_is_solo}")
        except Exception as e:
            logger.debug(f"⏳ rotation retry: سيعاد المحاولة للرقم {rec['phone_number']}: {e}")
        finally:
            try:
                await s2.disconnect()
            except Exception:
...
def seed_menu_items(menu: str):
    ...
    if menu == "main":
        ...
        with db_conn() as c:
            # ترقية أسماء الزر القديمة فقط؛ لا نلمس أي اسم مخصص اختاره المالك.
            c.execute(
                "UPDATE menu_items SET label=?, width=1 "
                "WHERE menu='main' AND action_value='services_menu' "
                "AND label IN (?, ?)",
                ("رشق حقيقي", "🐺 خدمات", "🛍 خدمات")
            )
...
def render_mb_menu_screen(menu: str):
    ...
    for it in items:
        ...
        rows.append([
            ...
            InlineKeyboardButton("✏️ تعديل الاسم", callback_data=f"mb_rename:{menu}:{it['id']}"),
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "await_mb_rename" and is_own:
        ...
        new_label = text.strip().replace("\n", " ")
        if not menu or not mid:
            context.user_data["state"] = "main_menu"
            await update.message.reply_text(
                "⚠️ انتهت جلسة تعديل الاسم، ابدأ من جديد.",
                reply_markup=owner_settings_kb(),
            )
            return
        if not new_label:
            await update.message.reply_text("⚠️ أرسل اسماً غير فارغ للزر.")
            return
        if len(new_label) > 64:
            await update.message.reply_text("⚠️ اسم الزر طويل جداً. الحد الأقصى 64 حرفاً.")
            return
        with db_conn() as c:
            row = c.execute(
        ...
        await update.message.reply_text(
            f"✅ تم تغيير اسم الزر من «{old_label}» إلى «{new_label}».\n"
            "سيظهر الاسم الجديد عند المالك والأعضاء.",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 رجوع لإدارة الأزرار", callback_data=f"mb_menu:{menu}")
            ]]),
        )
        return
...
async def _import_one_session_bytes(
    ...
    # ─── تدوير الجلسة تلقائياً فور الاستيراد ─────────────────────────
    # يجعل ملف الجلسة الأصلي عديم الفائدة فوراً بعد الرفع
    _bot_ref = getattr(getattr(context, "application", None), "bot", None)
    asyncio.create_task(rotate_session_safe(phone, stock_id, _bot_ref))
...
async def rotate_session_safe(phone: str, stock_id: int, bot_ref=None) -> bool:
    """
    تدوير الجلسة الآمن تلقائياً بعد كل استيراد جلسة.
    ينشئ جلسة جديدة S2 عبر QR، يحفظها في DB، ثم يقتل S1 القديمة.
    بعد النجاح يصبح ملف الجلسة الأصلي عديم الفائدة تماماً.
    يُرجع True عند النجاح الكامل، False عند الفشل.
    """
    s1 = None
    s2 = None
    try:
        with db_conn() as _rc:
            rec = _rc.execute(
                "SELECT session_string FROM number_stock WHERE id=%s", (stock_id,)
            ).fetchone()
        if not rec or not rec["session_string"]:
            logger.warning(f"🔄 rotate_safe: لا توجد جلسة للرقم {phone} — تم التخطي")
            return False

        s1 = TelegramClient(StringSession(rec["session_string"]), int(TELEGRAM_API_ID), TELEGRAM_API_HASH)
        s2 = TelegramClient(StringSession(), int(TELEGRAM_API_ID), TELEGRAM_API_HASH)

        await asyncio.wait_for(s1.connect(), timeout=15)
        await asyncio.wait_for(s2.connect(), timeout=15)

        if not await asyncio.wait_for(s1.is_user_authorized(), timeout=8):
            logger.warning(f"🔄 rotate_safe: S1 غير مصرّح للرقم {phone} — تم التخطي")
            return False

        # ─── المرحلة 1: S2 تولّد رمز QR ─────────────────────────────
        export_result = await asyncio.wait_for(
            s2(ExportLoginTokenRequest(
        ...
        if not isinstance(export_result, LoginToken):
            logger.warning(f"🔄 rotate_safe: توليد QR أرجع نوعاً غير متوقع للرقم {phone}")
            return False

        # ─── المرحلة 1.5: S1 تستورد رمز QR وتصرّح لـ S2 ─────────────
        try:
            await asyncio.wait_for(
                s1(ImportLoginTokenRequest(token=export_result.token)),
                timeout=15
            )
        except Exception:
            try:
                ...
                await asyncio.wait_for(
                    s1(AcceptLoginTokenRequest(token=export_result.token)),
                    timeout=10
                )
            except Exception as _acc_e:
                logger.warning(f"🔄 rotate_safe: فشل استيراد QR للرقم {phone}: {_acc_e}")
                return False

        await asyncio.sleep(2)

        # ─── المرحلة 2: التحقق من S2 ─────────────────────────────────
        s2_authorized = False
        _me_s2 = None
        for _att in range(3):
            try:
                s2_authorized = await asyncio.wait_for(s2.is_user_authorized(), timeout=8)
                if s2_authorized:
                    _me_s2 = await asyncio.wait_for(s2.get_me(), timeout=10)
                    break
            except Exception:
                pass
            if not s2_authorized:
                await asyncio.sleep(2)

        if not s2_authorized or not _me_s2:
            logger.warning(f"🔄 rotate_safe: S2 لم تتحقق للرقم {phone} — S1 سليمة")
            return False

        # ─── المرحلة 3: حفظ S2 في DB قبل أي شيء ──────────────────────
        s2_string = s2.session.save()
        with db_conn() as _db:
            _db.execute(
                "UPDATE number_stock SET session_string=%s, session_rotation_pending=TRUE WHERE id=%s",
                (s2_string, stock_id)
            )

        # ─── المرحلة 4: S2 تقتل S1 عبر ResetAuthorizations ──────────
        _reset_ok = False
        try:
            await asyncio.wait_for(s2(ResetAuthorizationsRequest()), timeout=20)
            _reset_ok = True
        except Exception as _rst:
            logger.warning(f"🔄 rotate_safe: ResetAuth فشل للرقم {phone}: {_rst} — المهمة الدورية ستُكمل")

        _is_solo = False
        if _reset_ok:
            try:
                _cnt = await asyncio.wait_for(get_device_count(s2), timeout=8)
                _is_solo = (_cnt == 1)
            except Exception:
                pass

        with db_conn() as _db2:
            if _reset_ok:
                _db2.execute(
                    "UPDATE number_stock SET session_rotation_pending=FALSE, sessions_reset=TRUE, is_solo=%s WHERE id=%s",
                    (_is_solo, stock_id)
        ...
        # ─── إشعار المالك بنتيجة التدوير ─────────────────────────────
        if bot_ref and OWNER_ID:
            _name = getattr(_me_s2, "first_name", None) or phone
            if _reset_ok:
                _msg = (
                    f"🔄✅ *تدوير الجلسة التلقائي اكتمل*\n\n"
                    f"📱 الرقم: `{phone}`\n"
                    f"👤 الحساب: {_name}\n\n"
                    f"🔒 الجلسة الأصلية أُتلفت نهائياً\n"
                    f"✅ الجلسة الجديدة نشطة وحدها\n"
                    f"🛡 ملف الجلسة القديم عديم الفائدة تماماً"
                )
            else:
                _msg = (
                    f"🔄⏳ *تدوير الجلسة — S2 محفوظة، ResetAuth معلّق*\n\n"
                    f"📱 الرقم: `{phone}`\n\n"
                    f"✅ S2 محفوظة في قاعدة البيانات\n"
                    f"⏳ S1 ستُقتل خلال 10 دقائق تلقائياً"
                )
            try:
                await bot_ref.send_message(OWNER_ID, _msg, parse_mode="Markdown")
            except Exception:
                pass

        logger.info(f"🔄✅ rotate_safe: تدوير الجلسة لـ {phone} | reset_ok={_reset_ok} | is_solo={_is_solo}")
        return True

    except asyncio.TimeoutError:
        logger.warning(f"🔄 rotate_safe: انتهت المهلة أثناء تدوير الجلسة للرقم {phone}")
        return False
    except Exception as _e:
        logger.error(f"🔄 rotate_safe: خطأ غير متوقع للرقم {phone}: {_e}")
        return False
    finally:
        for _cl in [s1, s2]:
            if _cl is not None:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "owner_settings" and is_own:
        if context.user_data.get("state", "").startswith("await_mb_"):
            context.user_data["state"] = "main_menu"
            for k in ("mb_menu", "mb_type", "mb_label", "mb_rename_menu", "mb_rename_id"):
    ...
    if data.startswith("mb_rename:") and is_own:
        _, rest = data.split(":", 1)
        ...
        await q.edit_message_text(
            f"✏️ *تعديل اسم الزر*\n\n"
            f"الاسم الحالي: *{row['label']}*\n\n"
            "أرسل الاسم الجديد كما تريد أن يظهر في القائمة الرئيسية:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 إلغاء", callback_data=f"mb_menu:{menu}")
            ]]),
        )
        return
    ...
    if data.startswith("os:number_info:") and is_own:
        ...
        try:
            ...
            kb_rows.append([InlineKeyboardButton("🔄 تدوير الجلسة (QR آمن — بدون فقدان الحساب)", callback_data=f"os:rotate_session:{stock_id}")])
    ...
    # ─── تدوير الجلسة (4 مراحل آمنة عبر QR) ───
    if data.startswith("os:rotate_session:") and is_own:
        stock_id = int(data.split(":")[-1])
        rec = get_stock_number(stock_id)
        ...
        phone = rec["phone_number"]
        await q.edit_message_text(
            f"⏳ *جاري تدوير جلسة {phone}...*\n\n"
            "📋 البروتوكول الآمن (4 مراحل):\n"
            "1️⃣ إنشاء S2 عبر QR (بدون 2FA)\n"
            "2️⃣ التحقق من S2 تعمل\n"
            "3️⃣ حفظ S2 في قاعدة البيانات\n"
            "4️⃣ S2 تقتل S1 عبر ResetAuthorizations\n\n"
        ...
        s2 = None
        try:
            # ─── المرحلة 1: إنشاء S2 وتوليد رمز QR ───────────────────────
            s1 = TelegramClient(StringSession(rec["session_string"]), int(TELEGRAM_API_ID), TELEGRAM_API_HASH)
            s2 = TelegramClient(StringSession(), int(TELEGRAM_API_ID), TELEGRAM_API_HASH)
            ...
            # التحقق من S1 لا تزال صالحة
            s1_ok = await asyncio.wait_for(s1.is_user_authorized(), timeout=8)
            if not s1_ok:
                await q.edit_message_text(
                    f"⚠️ *الجلسة الحالية (S1) للرقم {phone} منتهية.*\n\n"
                    "لا يمكن التدوير — البوت غير مصرّح له بهذا الحساب.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")]])
                )
                return

            # S2 تولّد رمز QR (ExportLoginToken)
            export_result = await s2(ExportLoginTokenRequest(
                api_id=int(TELEGRAM_API_ID),
                api_hash=TELEGRAM_API_HASH,
                except_ids=[]
            ))
            ...
            if not isinstance(export_result, LoginToken):
                await q.edit_message_text(
                    f"❌ *فشل توليد رمز QR للرقم {phone}.*\n\nالنوع المُرجَع غير متوقع.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")]])
                )
                return

            # ─── المرحلة 1.5: S1 تستورد رمز QR وتصرّح لـ S2 برمجياً ──────
            try:
                ...
                # بعض الحسابات تتطلب AcceptLoginToken — نجرّب البديل
                try:
                    from telethon.tl.functions.auth import AcceptLoginTokenRequest
                    import_result = await asyncio.wait_for(
                        s1(AcceptLoginTokenRequest(token=export_result.token)),
                    ...
                    await q.edit_message_text(
                        f"❌ *فشلت المرحلة 1 للرقم {phone}*\n\n"
                        f"تعذّر استيراد رمز QR برمجياً:\n`{str(_acc_err)[:200]}`\n\n"
                        "⚠️ S1 لا تزال سليمة — لم يتأثر الحساب.",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")]])
                    )
                    return

            # انتظر لحظة لكي تكتمل المصادقة
            await asyncio.sleep(2)

            # ─── المرحلة 2: التحقق من S2 تعمل بشكل كامل ────────────────
            s2_authorized = False
            _me_s2 = None
            ...
            if not s2_authorized or not _me_s2:
                await q.edit_message_text(
                    f"⚠️ *المرحلة 2 فشلت — S2 لم تتحقق للرقم {phone}*\n\n"
                    "✅ S1 لا تزال سليمة تماماً — لم يتأثر الحساب.\n"
                    "جرّب مجدداً أو تحقق من مشكلة الشبكة.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 إعادة المحاولة", callback_data=f"os:rotate_session:{stock_id}")],
                                                       [InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")]])
                )
                return

            # ─── المرحلة 3: حفظ S2 في DB أولاً (ضمان لا خسارة) ──────────
            s2_string = s2.session.save()
            with db_conn() as _db:
                _db.execute(
                    "UPDATE number_stock SET session_string=%s, session_rotation_pending=TRUE WHERE id=%s",
            ...
            # ─── المرحلة 4: S2 تستدعي ResetAuthorizations → تتلف S1 ──────
            _reset_ok = False
            try:
                await asyncio.wait_for(s2(ResetAuthorizationsRequest()), timeout=20)
                _reset_ok = True
            except Exception as _rst_err:
                logger.warning(f"⚠️ rotate: ResetAuth فشل للرقم {phone}: {_rst_err} — سيُعاد تلقائياً")

            _dev_cnt = -1
            _is_solo = False
            if _reset_ok:
                try:
                    _dev_cnt = await asyncio.wait_for(get_device_count(s2), timeout=8)
                    _is_solo = (_dev_cnt == 1)
                except Exception:
                    pass
            ...
            with db_conn() as _db2:
                ...
                # إذا فشل ResetAuth → session_rotation_pending تبقى TRUE للمهمة الدورية

            # ─── النتيجة النهائية ───────────────────────────────────────
            _name = getattr(_me_s2, "first_name", None) or phone
            if _reset_ok:
                await q.edit_message_text(
                    f"🔄✅ *اكتمل تدوير الجلسة بنجاح!*\n\n"
                    f"📱 الرقم: `{phone}`\n"
                    f"👤 الحساب: {_name}\n\n"
                    f"🔒 الجلسة القديمة S1 أُتلفت — S2 الجديدة نشطة وحدها\n"
                    f"💾 S2 محفوظة في قاعدة البيانات\n"
                    f"{'✅ البوت الجلسة الوحيدة' if _is_solo else '⚠️ ربما هناك جلسات إضافية'}",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([
                ...
                await q.edit_message_text(
                    f"🔄⏳ *تم حفظ S2 — ResetAuth سيُعاد تلقائياً*\n\n"
                    f"📱 الرقم: `{phone}`\n\n"
                    f"✅ S2 الجديدة محفوظة في قاعدة البيانات\n"
                    f"⏳ تيليجرام لم يقبل ResetAuth الآن — المهمة الدورية ستُكمل طرد S1 خلال 10 دقائق تلقائياً.\n"
                    f"⚠️ S1 قد تكون لا تزال نشطة حتى يتم الطرد.",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔙 رجوع لمعلومات الرقم", callback_data=f"os:number_info:{stock_id}")],
                    ])
                )
            logger.info(f"🔄 session rotation: {phone} | reset_ok={_reset_ok} | is_solo={_is_solo}")

        except asyncio.TimeoutError:
            await q.edit_message_text(
                f"⏳ *انتهت مهلة الاتصال أثناء تدوير الجلسة للرقم {phone}*\n\n"
                "⚠️ إذا كانت المرحلة 3 قد اكتملت، ستُكمل المهمة الدورية العملية تلقائياً.\n"
                "وإلا، S1 لا تزال سليمة.",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 إعادة المحاولة", callback_data=f"os:rotate_session:{stock_id}")],
                    [InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")],
                ])
            )
        except Exception as _rot_err:
            logger.error(f"❌ rotate_session error للرقم {phone}: {_rot_err}")
            await q.edit_message_text(
                f"❌ *خطأ غير متوقع أثناء تدوير الجلسة للرقم {phone}*\n\n"
                f"`{str(_rot_err)[:200]}`\n\n"
                "⚠️ تحقق من سجل DB لمعرفة ما إذا كانت S2 قد حُفظت أم لا.",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 إعادة المحاولة", callback_data=f"os:rotate_session:{stock_id}")],
                    [InlineKeyboardButton("🔙 رجوع", callback_data=f"os:number_info:{stock_id}")],
...
def main():
    ...
    if app.job_queue:
        ...
        app.job_queue.run_repeating(retry_pending_session_rotations_job, interval=600, first=120)
        logger.info("🔄 تم تفعيل إكمال تدوير الجلسات المعلّقة (كل 10 دقائق)")
