...
def raksh_menu_kb(is_owner: bool = False):
    ...
    for key, svc in RAKSH_SERVICES.items():
        ...
        if is_owner:
            ...
            buttons.append([service_button, toggle_button])
        else:
...
async def handle_raksh_callback(
    ...
    if data.startswith("raksh:toggle:") and not is_own:
    ...
    if data.startswith("raksh:start:"):
        ...
        svc = RAKSH_SERVICES.get(service_type)
        if not svc:
            await query.edit_message_text("⚠️ خدمة غير موجودة.", reply_markup=raksh_menu_kb(is_own))
            return
