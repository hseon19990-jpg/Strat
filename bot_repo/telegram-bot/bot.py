...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state != "verify_math" and not _owner_admin_state and not _sv_admin_state:
        try:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data not in _GATE_EXEMPT and not data.startswith("join_verify:") and not _owner_admin_action and not _sv_admin_action:
        try:
            _db_user = get_user(user.id)
            if _db_user and _db_user.get("verified", 0):
                _unjoined = await get_unjoined_mandatory_channels(context, user.id)
