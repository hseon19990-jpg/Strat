...
def list_stock_numbers(filter_type: str = "all"):
    ...
    if filter_type == "trash":
        ...
        sql = (
            "SELECT id, phone_number, session_string, is_solo, can_send_code, last_device_count, added_at "
            "FROM number_stock WHERE assigned_to IS NULL AND deleted_at IS NULL AND ever_sold IS NOT TRUE "
            "AND last_authorized IS NOT FALSE AND session_string IS NOT NULL "
...
async def solve_captcha_with_ai(client, bot_entity, msgs: list, phone: str = "", max_rounds: int = 4) -> tuple:
    ...
    def _gemini_image(prompt: str, img_bytes: bytes) -> str | None:
        try:
        ...
        for msg in msgs:
            ...
            if has_media:
                try:
                    ...
                    answer = _gemini_image(prompt, img_bytes)
                    if answer:
            ...
            if any(k in msg_text_lower for k in REACTION_KW):
                try:
                    ...
                    emoji_answer = _gemini_text(prompt)
                    if emoji_answer:
                        # خذ أول إيموجي فقط
...
async def do_referral_for_number(phone: str, session_str: str, bot_username: str, start_param: str,
    ...
    try:
        ...
        await asyncio.sleep(3)
        msgs = await client.get_messages(bot_entity, limit=8)

        # ── الخطوة 4: التعامل مع اشتراط البوت الانضمام لقنواته ──
        # (يجب أن يحدث قبل حل التحقق — لأن معظم البوتات تتحقق من الاشتراك أولاً)
        ...
        if joined_channels > 0:
            ...
            await asyncio.sleep(1.5)
            # بعد الانضمام، ابحث عن زر "تحقق من الاشتراك" واضغطه
            clicked = await _click_check_subscription_button(client, bot_entity, msgs)
            if clicked:
                steps.append("ضغط زر التحقق من الاشتراك")
            await asyncio.sleep(3)
            # احصل على رسائل البوت الجديدة بعد التحقق من الاشتراك
            msgs = await client.get_messages(bot_entity, limit=8)

        # ── الخطوة 5: حل التحقق (كابتشا) ──
        # "بتحقق" (use_ai=True)  → يحاول حل أي تحقق يطلبه البوت بالذكاء الاصطناعي
        # "بدون تحقق" (use_ai=False) → يتجاوز التحقق تماماً ويُعدّ الحساب ناجحاً
        if use_ai:
    ...
    for idx, entry in enumerate(sessions):
        ...
        try:
            ...
            if rot_ok:
                ...
                asyncio.create_task(_start_number_monitor(phone, final_sess, context.application))
                ok_list.append(f"{phone} 🔁")
            else:
                asyncio.create_task(_start_number_monitor(phone, sess, context.application))
    ...
    if data == "os:rotate_sessions" and is_own:
        ...
        for idx_r, rec_r in enumerate(_rows):
            ...
            if suc_r:
                ...
                asyncio.create_task(_start_number_monitor(ph_r, res_r, context.application))
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # payload: forced_ref_stars:{user_id}:{qty}:{total_stars}:{use_ai}:{cost_pts_channels}
        ...
        await _maybe_send_to_group(
            ...
            f'🔑 *طلب إحالة بوت اجباري (نجوم)*\n👤 {user.id}\n📌 `@{bot_user}` | `{start_p or "بدون كود"}`\n🔢 {qty} | ⭐ {total_stars:,} نجمة | 💎 {cost_pts_ch:,} نقطة (قنوات)\n🎫 `{code}`',
            parse_mode='Markdown'
        )
