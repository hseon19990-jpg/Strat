...
function Posts() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); create.mutate({ data: { campaignId: Number(data.get('campaignId')), channel: String(data.get('channel')), content: String(data.get('content')), targetReactions: Number(data.get('targetReactions')) } }, { onSuccess: () => { setCreateOpen(false); setNotice('تمت إضافة المنشور إلى الطابور.'); qc.invalidateQueries({ queryKey: getListPostsQueryKey({ status: 'all' }) }); qc.invalidateQueries({ queryKey: getGetDashboardSummaryQueryKey() }); } }); };
